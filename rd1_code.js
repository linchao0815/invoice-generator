...
1.4.8  linchao: 增加 Google Drive API 重試機制 (executeWithRetry)，處理「服務錯誤：雲端硬碟」等暫時性錯誤，連續失敗 3 次後會彈出視窗詢問是否繼續重試。
*================================================================================================================*/
...
function generateInvoice(bShowDialog = true) {
    try {
        ...
        var { rootFolder: rootInvoicesFolder, maxCounterMap: maxInvoicesCounterMap } = executeWithRetry(function() { return getFolderInfo(folderIdMatch, "Invoices"); });
        var { rootFolder: rootCreditNoteFolder, maxCounterMap: maxCreditNoteCounterMap } = executeWithRetry(function() { return getFolderInfo(folderIdMatch, "Credit Note"); });
        for (var i = 1; i < sheetValues.length; i++) {
            ...
            if (!rowData[pdfIndex] && companySettings) {
                ...
                if (!targetFolderId) {
                    // 先檢查是否已存在同名公司資料夾
                    targetFolderId = executeWithRetry(function() { return createOrRetrieveFolder(rootHandleFolder, currentCompanyName, targetFolderId, settingsSheet, companySettings, targetFolderIdColIdx); });
                    if (isCreditNote) {
                ...
                var invoiceId = executeWithRetry(function() { return DriveApp.getFileById(docId).makeCopy('Template Copy ' + new Date().getTime()).getId(); });
                var docBody = executeWithRetry(function() { return DocumentApp.openById(invoiceId).getBody(); });
                ...
                executeWithRetry(function() { DocumentApp.openById(invoiceId).setName(pdfName).saveAndClose(); });

                var pdfInvoice = executeWithRetry(function() { return convertPDF(invoiceId, targetFolderId); }); // Use the determined Folder URL
                dataSheet.getRange(i + 1, pdfIndex + 1).setValue(pdfInvoice[0]);
                rowData[pdfIndex] = pdfInvoice[0]; // 更新 rowData 以便寫入 log
                executeWithRetry(function() { Drive.Files.remove(invoiceId); });
    ...
    try {
        ...
        for (var i = 1; i < sheetValues.length; i++) {
            ...
            if (pdfUrl && !emailSentStatus && recipientColIndex !== -1) {
                ...
                // 複製範本並填入資料
                var emailDocId = executeWithRetry(function() { return DriveApp.getFileById(templateId).makeCopy('Email Template Copy ' + new Date().getTime()).getId(); });
                var docBody = executeWithRetry(function() { return DocumentApp.openById(emailDocId).getBody(); });
                ...
                executeWithRetry(function() { DocumentApp.openById(emailDocId).saveAndClose(); });
                ...
                // 刪除臨時文件
                executeWithRetry(function() { Drive.Files.remove(emailDocId); });
                ...
                if (pdfFileId) {
                    try {
                        var pdfFile = executeWithRetry(function() { return DriveApp.getFileById(pdfFileId); });
                ...
                if (attFileId) {
                    try {
                        var attFile = executeWithRetry(function() { return DriveApp.getFileById(attFileId); });
...
/**
 * Execute a function with retry logic to handle transient Google Drive API errors
 * @param {Function} func The function to execute
 * @param {number} maxRetries Maximum number of retries (default 3)
 * @returns {any} The result of the function
 */
function executeWithRetry(func, maxRetries) {
    ...
    while (true) {
        try {
            ...
            if (attempt < maxRetries) {
                Utilities.sleep(1000 * attempt + Math.random() * 1000); // Exponential backoff with jitter
            } else {
                // 已達最大重試次數，詢問使用者是否繼續重試
                try {
                    var ui = SpreadsheetApp.getUi();
                    var response = ui.alert(
                        '重試失敗', 
                        '發生錯誤：\n' + e.message + '\n\n系統已自動重試 ' + maxRetries + ' 次仍然失敗。\n是否要繼續嘗試？', 
                        ui.ButtonSet.YES_NO
                    );
                    if (response === ui.Button.YES) {
                        attempt = 0; // 重置計數器，再重試一輪
                    } else {
                        throw lastError; // 選擇放棄，拋出錯誤
                    }
                } catch (uiErr) {
                    // 如果在沒有 UI 環境的地方執行 (例如 Trigger)，無法彈出視窗，直接拋出錯誤
                    throw lastError;
                }
            }
        }
